document.addEventListener("DOMContentLoaded", function () {
    ...
    function downloadChart() {
        ...
        const pdf = new jsPDF('p', 'pt', 'a4');
         ...
         // Banner Styling
    pdf.setFillColor(51, 51, 51); // Dark background color to match banner
    ...
    pdf.setFontSize(24);
    pdf.text('Player Evaluation Form', pdf.internal.pageSize.width / 2, 40, { align: 'center' });

    // Player Name and Date
    const formattedDate = evaluationDate || 'MM/DD/YYYY';
    const playerTitle = `${formattedDate} ${playerName}`;
    pdf.setTextColor(0, 0, 0); // Black text for player name and date
    pdf.setFont('Times', 'normal');
    pdf.setFontSize(16);
    pdf.text(playerTitle, 40, 100);
