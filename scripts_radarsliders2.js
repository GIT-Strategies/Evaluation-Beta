document.addEventListener("DOMContentLoaded", function () {
    ...
    function downloadChart() {
        ...
        // Generate PDF content without hidden elements
        html2canvas(rightSection).then(canvas => {
            const imgData = canvas.toDataURL('image/png');
    
            // Restore elements after snapshot
            ...
            // Draw vertical divider line between scores and comments
            const dividerX = 200;
            pdf.setDrawColor(0);
            pdf.line(dividerX, 640, dividerX, sliderYPosition + 20);
    
            // Comments Section with splitTextToSize for Wrapping
            const commentsText = document.getElementById('comments').value;
            const commentsX = dividerX + 10;
            const commentsY = 650;
            const maxWidth = pdf.internal.pageSize.width - commentsX - 40; // Allow full width usage with padding
            const lineHeight = 15;
    
            pdf.text('Comments:', commentsX, commentsY);
            pdf.setFont('Times', 'italic');
            
            // Use splitTextToSize for wrapping comments within maxWidth
            const wrappedComments = pdf.splitTextToSize(commentsText, maxWidth);
            pdf.text(wrappedComments, commentsX, commentsY + 20);
    
            // Footer Text: "Powered by Give it a Try" in Black
            pdf.setFont('Times', 'italic');
            pdf.setFontSize(10);
